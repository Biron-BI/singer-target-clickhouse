...
class ClickhouseConnection(private val config: TargetConfig) : TargetConnection {
	...
	override fun runQuery(query: String, retries: Int): QueryResult = withRetries(retries) {
	...
	override fun listTables(): List<String> =
		runQuery("SHOW TABLES").data.map { it[0].toString() }

	override fun listColumns(table: String): List<Column> {
		val res = runQuery(
	...
	override fun addColumn(table: String, newCol: Column): Either<AddColumnError, Unit> = try {
		logger.info { "[$table] Adding column $table.${newCol.name} ${newCol.type}" }
		runQuery("ALTER TABLE $table ADD COLUMN `${newCol.name}` ${newCol.type}", retries = 2)
		Unit.right()
	} catch (e: Throwable) {
		AddColumnError(newCol, e).left()
	}

	override fun removeColumn(table: String, existing: Column): Either<RemoveColumnError, Unit> = try {
		logger.info { "[$table] Removing column $table.${existing.name}" }
		runQuery("ALTER TABLE $table DROP COLUMN `${existing.name}`")
		Unit.right()
	} catch (e: Throwable) {
		RemoveColumnError(existing, e).left()
	}

	override fun updateColumn(table: String, existing: Column, newCol: Column): Either<UpdateColumnError, Unit> = try {
		logger.info { "[$table] Updating column $table.${existing.name} from ${existing.type} to ${newCol.type}" }
		runQuery("ALTER TABLE $table MODIFY COLUMN `${newCol.name}` ${newCol.type}", retries = 0)
		Unit.right()
	} catch (e: Throwable) {
		...
		try {
			runQuery("ALTER TABLE $table MODIFY COLUMN `${existing.name}` ${existing.type}")
	...
	override fun renameObsoleteColumn(table: String): QueryResult {
		...
		return runQuery("RENAME TABLE `$table` TO `${TargetConnection.DROPPED_TABLE_PREFIX}$table`")
	}

	override fun openRowWriter(query: String): RowWriter = HttpStreamingRowWriter(
		url = insertUrl(query),
 ...
 */
private class BlockingQueueInputStream : InputStream() {
...
private class HttpStreamingRowWriter(
	url: URI,
	authHeader: String,
	httpClient: HttpClient,
) : RowWriter {
...
/** A single-row line as the HTTP body writes expect (JSONCompactEachRow, newline-terminated). */
fun OutputStream.writeRowLine(line: String) {
