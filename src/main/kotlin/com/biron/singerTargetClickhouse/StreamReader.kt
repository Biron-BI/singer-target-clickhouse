...
class StreamReader(
	...
	val pkCount: Int,
	val columnCount: Int,
	val subtableCount: Int,
	/** Slot of the cleaning column (either a CURRENT pk or a simple column), or -1. */
	val cleaningColumnSlot: Int,
	private val root: FieldReader,
) {
	fun columnSlot(index: Int): Int = pkCount + index

	fun subtableSlot(index: Int): Int = pkCount + columnCount + index

	fun read(parser: JsonParser): RecordRow {
		val row = arrayOfNulls<Any?>(width)
		root.read(parser, row)
		return row
	}
}
 ...
 */
class ObjectReader(private val children: Map<String, FieldReader>) : FieldReader {
...
class UntypedScalarReader(private val slot: Int) : FieldReader {
	override fun read(parser: JsonParser, row: RecordRow) {
...
class StringReader(private val slot: Int) : FieldReader {
	override fun read(parser: JsonParser, row: RecordRow) {
...
/** schemaType == "integer": mirrors [translateValue] — Number→toLong, String→parseLong|toDouble.toLong, else null. */
class IntegerReader(private val slot: Int) : FieldReader {
	override fun read(parser: JsonParser, row: RecordRow) {
		row[slot] = when (parser.currentToken) {
...
/** schemaType == "number": mirrors [translateValue] — Number→toDouble, String→parseDouble, else null. */
class NumberReader(private val slot: Int) : FieldReader {
	override fun read(parser: JsonParser, row: RecordRow) {
		row[slot] = when (parser.currentToken) {
...
class BooleanReader(private val slot: Int) : FieldReader {
	override fun read(parser: JsonParser, row: RecordRow) {
		row[slot] = when (parser.currentToken) {
			JsonToken.VALUE_NULL -> null
 ...
 */
fun buildStreamReader(meta: SourceMeta, translateValues: Boolean): StreamReader {
	...
	if (columnCount == 1 && meta.simpleColumnMappings.single().prop == null && pkCount == 0 && subtableCount == 0) {
		...
		return StreamReader(
			...
			columnCount = 1,
			subtableCount = 0,
			cleaningColumnSlot = -1,
			root = scalarReaderFor(0, col.schemaType, col.nestedArray, translateValues),
		)
	}
	...
	meta.children.forEachIndexed { i, child ->
		builder.addLeaf(child.prop, SubtableReader(pkCount + columnCount + i, buildStreamReader(child, translateValues)))
	}
	...
	return StreamReader(
		...
		cleaningColumnSlot = computeCleaningSlot(meta.cleaningColumn, currentPks, meta.simpleColumnMappings, pkCount),
		root = builder.build(),
	)
}
...
private fun computeCleaningSlot(
	...
	if (cleaningColumn == null) return -1
	...
	return if (colIdx >= 0) pkCount + colIdx else -1
}
