...
A [Singer](https://singer.io/) target for Clickhouse, for use with Singer streams generated by Singer taps, written in
Kotlin on the JVM.
...
The artifact is produced at `build/libs/singer-target-clickhouse.jar` and can be invoked with `java -jar`.
...
A `Dockerfile` is included at the project root: run `./gradlew bootJar && docker build -t target-clickhouse .` to
...
**JAR:**
   ...
   ```bash
   <tap-anything> --state current_state.json | java -jar build/libs/singer-target-clickhouse.jar --config config.json >> state.jsonl
   ```
...
* `insert_stream_timeout_sec`: HTTP receive timeout for an open insert stream, in seconds. Default `180`
* `deletion_batch_size`: Amount of deletion records buffered before issuing the `ALTER … DELETE`. Default `100`
